//AUTOGENERATED FILE FROM png2mtspr

#include <sms/sms.h>
#include <sms/metasprites.h>

const UINT8 sprite_data[896] = {

};

const metasprite_t sprite_metasprite0[] = {
	{-24, -24, 0}, {0, 8, 2}, {0, 8, 4}, {0, 8, 6}, {0, 8, 8}, {0, 8, 0}, {16, -40, 10}, {0, 8, 12}, {0, 8, 14}, {0, 8, 16}, {0, 8, 16}, {0, 8, 18}, {0, 8, 20}, {16, -48, 22}, {0, 16, 24}, {0, 8, 26}, {0, 8, 28}, {0, 8, 30}, {0, 8, 32}, {metasprite_end}
};

const metasprite_t sprite_metasprite1[] = {
	{-24, -24, 0}, {0, 8, 2}, {0, 8, 4}, {0, 8, 6}, {0, 8, 8}, {0, 8, 0}, {16, -40, 10}, {0, 8, 12}, {0, 8, 14}, {0, 8, 16}, {0, 8, 16}, {0, 8, 18}, {0, 8, 34}, {16, -56, 36}, {0, 8, 38}, {0, 16, 24}, {0, 8, 26}, {0, 8, 28}, {0, 8, 30}, {0, 8, 40}, {metasprite_end}
};

const metasprite_t sprite_metasprite2[] = {
	{-24, -24, 0}, {0, 8, 2}, {0, 8, 4}, {0, 8, 6}, {0, 8, 8}, {0, 8, 0}, {16, -40, 10}, {0, 8, 12}, {0, 8, 14}, {0, 8, 16}, {0, 8, 16}, {0, 8, 18}, {0, 8, 42}, {16, -56, 44}, {0, 8, 46, 0}, {0, 16, 24}, {0, 8, 26}, {0, 8, 28}, {0, 8, 30}, {0, 8, 48}, {metasprite_end}
};

const metasprite_t sprite_metasprite3[] = {
	{-24, -24, 0}, {0, 8, 2}, {0, 8, 4}, {0, 8, 6}, {0, 8, 8}, {0, 8, 0}, {16, -40, 10}, {0, 8, 12}, {0, 8, 14}, {0, 8, 16}, {0, 8, 16}, {0, 8, 18}, {0, 8, 34}, {16, -56, 50}, {0, 8, 52}, {0, 16, 24}, {0, 8, 26}, {0, 8, 28}, {0, 8, 30}, {0, 8, 40}, {metasprite_end}
};

const metasprite_t sprite_metasprite4[] = {
	{-24, -24, 0}, {0, 8, 2}, {0, 8, 4}, {0, 8, 6}, {0, 8, 8}, {0, 8, 0}, {16, -40, 10}, {0, 8, 12}, {0, 8, 14}, {0, 8, 16}, {0, 8, 16}, {0, 8, 18}, {0, 8, 20}, {16, -56, 54}, {0, 8, 52}, {0, 16, 24}, {0, 8, 26}, {0, 8, 28}, {0, 8, 30}, {0, 8, 32}, {metasprite_end}
};

const metasprite_t* const sprite_metasprites[5] = {
	sprite_metasprite0, sprite_metasprite1, sprite_metasprite2, sprite_metasprite3, sprite_metasprite4
};
