...
void GetMap( PNG2AssetData* png2AssetData)
{
	for(int y = 0; y < (int)png2AssetData->image.h; y += png2AssetData->image.tile_h)
	{
		for(int x = 0; x < (int)png2AssetData->image.w; x += png2AssetData->image.tile_w)
		{
			...
			png2AssetData->image.ExtractTile(x, y, tile, png2AssetData->arguments.sprite_mode, png2AssetData->arguments.export_as_map, png2AssetData->arguments.use_map_attributes);
			...
			else
			{
				if(!FindTile(tile, idx, props,png2AssetData))
				{
					...
					if(png2AssetData->tiles.size() > 256 && png2AssetData->arguments.pack_mode != Tile::SMS)
						printf("Warning: found more than 256 tiles on x:%d,y:%d\n", x, y);

					if(((png2AssetData->tiles.size() + png2AssetData->arguments.tile_origin) > 256) && (png2AssetData->arguments.pack_mode != Tile::SMS))
						printf("Warning: tile count (%d) + tile origin (%d) exceeds 256 at x:%d,y:%d\n", (unsigned int)png2AssetData->tiles.size(), png2AssetData->arguments.tile_origin, x, y);
				}
			}

			png2AssetData->map.push_back((unsigned char)idx + png2AssetData->arguments.tile_origin);

			if(png2AssetData->arguments.use_map_attributes)
			{
				unsigned char pal_idx = png2AssetData->image.data[y * png2AssetData->image.w + x] >> png2AssetData->arguments.bpp; //We can pick the palette from the first pixel of this tile
				if(png2AssetData->arguments.pack_mode == Tile::SGB)
				{
				...
				else if(png2AssetData->arguments.pack_mode == Tile::SMS)
				{
					props = props >> 4;
					if(idx > 255)
						props |= 1;
