...
{
	if(x < png2AssetData->map_attributes_width && y < png2AssetData->map_attributes_height)
		return png2AssetData->map_attributes[y * png2AssetData->map_attributes_width + x];
	else
		return 0;
}
...
{
	size_t w = (png2AssetData->map_attributes_width + 1) / 2;
	size_t h = (png2AssetData->map_attributes_height + 1) / 2;
	...
	png2AssetData->map_attributes_width = w;
	png2AssetData->map_attributes_height = h;
...
void AlignMapAttributes(PNG2AssetData* png2AssetData)
{
	...
	size_t map_attributes_aligned_width = 2 * ((png2AssetData->map_attributes_width + 1) / 2);
	size_t num_vertical_nametables = (png2AssetData->map_attributes_height + ATTRIBUTE_HEIGHT - 1) / ATTRIBUTE_HEIGHT;
	...
	for(size_t i = 0; i < num_vertical_nametables; i++)
	{
		...
		size_t height = last_nametable ? (png2AssetData->map_attributes_height - i * ATTRIBUTE_HEIGHT) : ATTRIBUTE_HEIGHT;
		for(size_t y = 0; y < height; y++)
		{
			for(size_t x = 0; x < png2AssetData->map_attributes_width; x++)
			{
				map_attributes_aligned[(i * ATTRIBUTE_ALIGNED_HEIGHT + y) * map_attributes_aligned_width + x] =
					png2AssetData->map_attributes[(i * ATTRIBUTE_HEIGHT + y) * png2AssetData->map_attributes_width + x];
			}
		}
	}
	// Overwrite old attributes
	png2AssetData->map_attributes_width = map_attributes_aligned_width;
	png2AssetData->map_attributes_height = num_vertical_nametables * ATTRIBUTE_ALIGNED_HEIGHT;
...
void PackMapAttributes(PNG2AssetData *png2AssetData)
{
	...
	png2AssetData->map_attributes_packed_width = (png2AssetData->map_attributes_width + 1) / 2;
	png2AssetData->map_attributes_packed_height = (png2AssetData->map_attributes_height + 1) / 2;
	map_attributes_packed.resize(png2AssetData->map_attributes_packed_width * png2AssetData->map_attributes_packed_height);
	for(size_t y = 0; y < png2AssetData->map_attributes_packed_height; y++)
	{
		for(size_t x = 0; x < png2AssetData->map_attributes_packed_width; x++)
		{
			...
			map_attributes_packed[png2AssetData->map_attributes_packed_width * y + x] = packed_bits;
		}
	}
	// Overwrite old attributes
...
void HandleMapAttributes(PNG2AssetData* png2AssetData) {
	png2AssetData->map_attributes_width = png2AssetData->image.w / 8;
	png2AssetData->map_attributes_height = png2AssetData->image.h / 8;

	// Optionally perform 2x2 reduction on attributes (NES attribute table has this format)
	if(png2AssetData->use_2x2_map_attributes)
	{
	...
	if(png2AssetData->pack_map_attributes)
	{
	...
	else
	{
		...
		png2AssetData->map_attributes_packed_width = png2AssetData->map_attributes_width;
		png2AssetData->map_attributes_packed_height = png2AssetData->map_attributes_height;
	}
}