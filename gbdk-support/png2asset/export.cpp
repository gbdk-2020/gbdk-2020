...
int HandleExport( PNG2AssetData* png2AssetData) {
	...
	if((png2AssetData->export_as_map) && (png2AssetData->output_binary)) {
...
bool export_h_file( PNG2AssetData* png2AssetData) {
	...
	file = fopen(png2AssetData->output_filename_h.c_str(), "w");
	if(!file) {
		printf("Error writing file: %s", png2AssetData->output_filename_h.c_str());
		return false;
	}

	fprintf(file, "//AUTOGENERATED FILE FROM png2asset\n");
	fprintf(file, "#ifndef METASPRITE_%s_H\n", png2AssetData->data_name.c_str());
	...
	{
		fprintf(file, "#define %s_TILE_ORIGIN %d\n", png2AssetData->data_name.c_str(), png2AssetData->tile_origin);
		fprintf(file, "#define %s_TILE_W %d\n", png2AssetData->data_name.c_str(), png2AssetData->image.tile_w);
		fprintf(file, "#define %s_TILE_H %d\n", png2AssetData->data_name.c_str(), png2AssetData->image.tile_h);
		fprintf(file, "#define %s_WIDTH %d\n", png2AssetData->data_name.c_str(), png2AssetData->sprite_w);
		fprintf(file, "#define %s_HEIGHT %d\n", png2AssetData->data_name.c_str(), png2AssetData->sprite_h);
		...
		fprintf(file, "#define %s_TILE_COUNT %d\n", png2AssetData->data_name.c_str(), ((unsigned int)png2AssetData->tiles.size() - png2AssetData->source_tileset_size) * (png2AssetData->image.tile_h >> 3) * (png2AssetData->image.tile_w >> 3));
		if(png2AssetData->include_palettes) {
			fprintf(file, "#define %s_PALETTE_COUNT %d\n", png2AssetData->data_name.c_str(), (unsigned int)(png2AssetData->image.total_color_count / png2AssetData->image.colors_per_pal));
			fprintf(file, "#define %s_COLORS_PER_PALETTE %d\n", png2AssetData->data_name.c_str(), (unsigned int)png2AssetData->image.colors_per_pal);
			fprintf(file, "#define %s_TOTAL_COLORS %d\n", png2AssetData->data_name.c_str(), (unsigned int)png2AssetData->image.total_color_count);
		}

		if(png2AssetData->includedMapOrMetaspriteData) {

			if(png2AssetData->export_as_map)
			{
				fprintf(file, "#define %s_MAP_ATTRIBUTES ", png2AssetData->data_name.c_str());
				if(png2AssetData->use_map_attributes && png2AssetData->map_attributes.size())
				...
				{
					int scale = png2AssetData->use_2x2_map_attributes ? 2 : 1;
					fprintf(file, "#define %s_MAP_ATTRIBUTES_WIDTH %d\n", png2AssetData->data_name.c_str(), (int)(scale * png2AssetData->map_attributes_width));
					fprintf(file, "#define %s_MAP_ATTRIBUTES_HEIGHT %d\n", png2AssetData->data_name.c_str(), (int)(scale * png2AssetData->map_attributes_height));
					fprintf(file, "#define %s_MAP_ATTRIBUTES_PACKED_WIDTH %d\n", png2AssetData->data_name.c_str(), (int)png2AssetData->map_attributes_packed_width);
					fprintf(file, "#define %s_MAP_ATTRIBUTES_PACKED_HEIGHT %d\n", png2AssetData->data_name.c_str(), (int)png2AssetData->map_attributes_packed_height);
				}

				if(png2AssetData->use_structs)
				{
					fprintf(file, "#define %s_TILE_PALS ", png2AssetData->data_name.c_str());
					if(png2AssetData->use_map_attributes)
						fprintf(file, "0\n");
					else
			...
			{
				fprintf(file, "#define %s_PIVOT_X %d\n", png2AssetData->data_name.c_str(), png2AssetData->pivot_x);
				fprintf(file, "#define %s_PIVOT_Y %d\n", png2AssetData->data_name.c_str(), png2AssetData->pivot_y);
				fprintf(file, "#define %s_PIVOT_W %d\n", png2AssetData->data_name.c_str(), png2AssetData->pivot_w);
				fprintf(file, "#define %s_PIVOT_H %d\n", png2AssetData->data_name.c_str(), png2AssetData->pivot_h);
			}
		}
		fprintf(file, "\n");
		...
		if(png2AssetData->include_palettes && (png2AssetData->image.total_color_count - png2AssetData->source_total_color_count > 0 || !png2AssetData->use_source_tileset)) {
			fprintf(file, "extern const palette_color_t %s_palettes[%d];\n", png2AssetData->data_name.c_str(), (unsigned int)png2AssetData->image.total_color_count - png2AssetData->source_total_color_count);
		}
		if(png2AssetData->includeTileData) {
			fprintf(file, "extern const uint8_t %s_tiles[%d];\n", png2AssetData->data_name.c_str(), (unsigned int)((png2AssetData->tiles.size() - png2AssetData->source_tileset_size) * (png2AssetData->image.tile_w * png2AssetData->image.tile_h * png2AssetData->bpp / 8)));
		}
		...
		if(png2AssetData->includedMapOrMetaspriteData) {
			if(png2AssetData->export_as_map)
			{
				fprintf(file, "extern const unsigned char %s_map[%d];\n", png2AssetData->data_name.c_str(), (unsigned int)(png2AssetData->map).size());

				if(png2AssetData->use_map_attributes && png2AssetData->map_attributes.size()) {
					fprintf(file, "extern const unsigned char %s_map_attributes[%d];\n", png2AssetData->data_name.c_str(), (unsigned int)(png2AssetData->map_attributes).size());
				}
				else
				{
					...
					fprintf(file, "#define %s_map_attributes %s_map\n", png2AssetData->data_name.c_str(), png2AssetData->data_name.c_str());
				}
				if(!png2AssetData->use_map_attributes && (png2AssetData->includeTileData) && (png2AssetData->use_structs)) {
					fprintf(file, "extern const unsigned char* %s_tile_pals[%d];\n", png2AssetData->data_name.c_str(), (unsigned int)(png2AssetData->tiles).size());
				}
			}
			else
			{
				fprintf(file, "extern const metasprite_t* const %s_metasprites[%d];\n", png2AssetData->data_name.c_str(), (unsigned int)(png2AssetData->sprites.size()));
			}
		}
	}
	fprintf(file, "\n");
...
bool export_c_file( PNG2AssetData* png2AssetData) {
	...
	file = fopen(png2AssetData->output_filename.c_str(), "w");
	if(!file) {
		printf("Error writing file: %s", png2AssetData->output_filename.c_str());
		return false;
	}

	if(png2AssetData->bank >= 0) fprintf(file, "#pragma bank %d\n\n", png2AssetData->bank);
	...
	if(png2AssetData->include_palettes && (png2AssetData->image.total_color_count - png2AssetData->source_total_color_count > 0 || !png2AssetData->use_source_tileset)) {
		...
		for(size_t i = png2AssetData->source_total_color_count / png2AssetData->image.colors_per_pal; i < png2AssetData->image.total_color_count / png2AssetData->image.colors_per_pal; ++i)
		{
			if(i != 0)
				fprintf(file, ",\n");
			fprintf(file, "\t");
			...
			for(int c = 0; c < (int)png2AssetData->image.colors_per_pal; ++c, pal_ptr += RGBA32_SZ)
			{
				size_t rgb222 = (((pal_ptr[2] >> 6) & 0x3) << 4) |
					(((pal_ptr[1] >> 6) & 0x3) << 2) |
					(((pal_ptr[0] >> 6) & 0x3) << 0);
				if(png2AssetData->convert_rgb_to_nes)
	...
	if(png2AssetData->includeTileData) {
		...
		fprintf(file, "\t");
		for(vector< Tile >::iterator it = png2AssetData->tiles.begin() + png2AssetData->source_tileset_size; it != png2AssetData->tiles.end(); ++it)
		{

			int line_break = 1; // Start with 1 to prevent line break on first iteration
			vector< unsigned char > packed_data = (*it).GetPackedData(png2AssetData->pack_mode, png2AssetData->image.tile_w, png2AssetData->image.tile_h, png2AssetData->bpp);
			for(vector< unsigned char >::iterator it2 = packed_data.begin(); it2 != packed_data.end(); ++it2)
			{
				...
				if(((line_break++) % (8 / png2AssetData->bpp)) == 0)
					fprintf(file, "\n\t");
			}

			if((!png2AssetData->export_as_map) && (it != png2AssetData->tiles.end()))
				fprintf(file, "\n");
		}
		fprintf(file, "};\n\n");
	}
	...
	if(png2AssetData->includedMapOrMetaspriteData) {
		...
		if(!png2AssetData->export_as_map)
		{
			...
			{
				fprintf(file, "const metasprite_t %s_metasprite%d[] = {\n", png2AssetData->data_name.c_str(), (int)(it - png2AssetData->sprites.begin()));
			...
			for(vector< MetaSprite >::iterator it = png2AssetData->sprites.begin(); it != png2AssetData->sprites.end(); ++it)
			{
				fprintf(file, "%s_metasprite%d", png2AssetData->data_name.c_str(), (int)(it - png2AssetData->sprites.begin()));
			...
			if(png2AssetData->use_structs)
			{
				...
				fprintf(file, "\t%d, //width\n", png2AssetData->pivot_w);
				fprintf(file, "\t%d, //height\n", png2AssetData->pivot_h);
		...
		else
		{
			if(png2AssetData->includeTileData) {
				if(png2AssetData->use_structs)
				{
					...
					{
						fprintf(file, "\n");
						fprintf(file, "const uint8_t %s_tile_pals[%d] = {\n\t", png2AssetData->data_name.c_str(), (unsigned int)(png2AssetData->tiles.size()) - png2AssetData->source_tileset_size);
						for(vector< Tile >::iterator it = png2AssetData->tiles.begin() + png2AssetData->source_tileset_size; it != png2AssetData->tiles.end(); ++it)
						{
			...
			if(png2AssetData->output_transposed) {
			...
			if(png2AssetData->use_map_attributes && png2AssetData->map_attributes.size())
			{
				...
				if(png2AssetData->output_transposed) {
					for(size_t i = 0; i < png2AssetData->map_attributes_packed_width; ++i)
					{
						fprintf(file, "\t");
						for(size_t j = 0; j < png2AssetData->map_attributes_packed_height; ++j)
						{
							fprintf(file, "0x%02x,", png2AssetData->map_attributes[j * png2AssetData->map_attributes_packed_width + i]);
						}
						fprintf(file, "\n");
					}
				}
...
bool export_map_binary( PNG2AssetData* png2AssetData) {
	...
	mapBinaryFile.open(png2AssetData->output_filename_bin, std::ios_base::binary);
	tilesBinaryFile.open(png2AssetData->output_filename_tiles_bin, std::ios_base::binary);
	...
	if(png2AssetData->use_map_attributes)mapAttributesBinaryfile.open(png2AssetData->output_filename_attributes_bin, std::ios_base::binary);
	...
	if(png2AssetData->output_transposed) {
		...
		if(png2AssetData->use_map_attributes)mapAttributesBinaryfile.write((const char*)(&png2AssetData->map_attributes[0]), rows * columns);
	}

	// Finalize the files
	mapBinaryFile.close();
	tilesBinaryFile.close();
	if(png2AssetData->use_map_attributes)mapAttributesBinaryfile.close();
